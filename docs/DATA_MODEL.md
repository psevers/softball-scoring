...
A correction session may retain multiple pitch edits, pitch deletions, and eligible non-scoring Ball In Play replacements keyed by their original record IDs. Each candidate rebuild preserves original order and envelope metadata for surviving records. Saving applies all payload replacements and deletions through one isolated SwiftData context and one save; no intermediate candidate is durable.
