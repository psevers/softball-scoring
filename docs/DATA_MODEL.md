...
A correction session may retain defensive or tracked-team pitch edits and deletions, eligible Ball In Play replacements, and paired logical-play deletions keyed by their original record IDs. Each candidate rebuild preserves original order and envelope metadata for surviving records. Saving applies all payload replacements and deletions through one isolated SwiftData context and one save; no intermediate candidate is durable.
...
A completed non-third-out tracked-team plate-appearance correction also replaces only its event payload while preserving the record envelope. Existing runner destinations, `countedRunSources`, and RBI fields encode the corrected score and exact event-time player attribution. Replay and batting projection derive every affected runner run and batter RBI, including removing both when a Home destination moves back to a base or Out; no durable schema change is required.
