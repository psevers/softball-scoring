...
- An unknown-kind or malformed-payload problem record may be deleted only by exact record identity after explicit deletion-only confirmation. The scorer never guesses or edits the missing event meaning. The candidate must pass full decode, semantic validation, replay, and batting projection with no remaining rejected record before one atomic save removes the target. Survivors keep their IDs, sequence numbers, timestamps, and gaps; invalid-sequence and semantic-rejection problems are not eligible for this recovery action.
