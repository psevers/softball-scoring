...
- Deleting an earlier tracked-team count pitch stages removal of only that event record. Confirmation uses its event-time player, batting slot and order size, result, and sequence. Full replay reconstructs the active count, tracked batter, and every surviving player-attributed result; an invalid downstream tracked pitch is never rewritten or skipped and keeps Save disabled until explicitly edited or deleted.
...
- remaining correction workflows beyond delivered pitch and completed defensive-play correction,
- pitching changes,
