...
- Editing an eligible completed defensive Ball In Play reuses the ordinary runner-movement validator. The scorer confirms the batter and every occupied event-time runner exactly once, explicitly confirms every home touch that legally counts on a non-third-out play, and assigns RBI no greater than counted runs. Missing, duplicate, unexpected, backward, passing, colliding, excess-out, outcome-inconsistent, illegal run-count, and excess-RBI proposals are rejected. Single-out/non-third-out scoring plays are eligible; multi-out results and plays that create the third out remain unavailable until their dedicated correction slices. The counted In Play pitch and pitcher total do not change.
