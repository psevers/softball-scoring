...
- Editing a completed non-scoring defensive Ball In Play reuses the ordinary runner-movement validator. The scorer confirms the batter and every occupied event-time runner exactly once. Missing, duplicate, unexpected, backward, passing, colliding, excess-out, and outcome-inconsistent movement is rejected. Home destinations, multi-out results, and plays that create the third out remain unavailable until their dedicated correction slices. The counted In Play pitch and pitcher total do not change.
