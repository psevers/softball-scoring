...
- A locked timeline is repaired one explicit problem at a time in one correction session. Known decodable events use only their supported editor or deletion control. Unknown-kind, malformed-payload, and invalid or duplicate-sequence problems may be deleted only by exact record identity; the scorer never guesses or edits missing meaning. Every stage rebuilds the complete candidate, and Save remains unavailable until full decode, semantic validation, replay, History, and batting projection have no remaining problem. One atomic save applies the batch. Survivors keep their IDs, event bodies, sequence numbers, timestamps, chronological order, and sequence gaps; no record is silently skipped, rewritten, renumbered, detached, or cascade-deleted.
...
- Deleting a completed tracked-team logical play removes every count-pitch component and its terminal plate-appearance result by exact record identity. Unrelated SB/CS records interleaved with the play survive. Full replay reconstructs count, bases, score, outs, current tracked batter, and batting projection without the deleted result, runs, or RBI; any rejected downstream record keeps Save disabled until explicitly repaired in the same atomic session.
- Editing an earlier completed tracked-team plate appearance supports walk, HBP, strikeout, hit, home-run, error, fielder's-choice, ordinary-out, sacrifice, double-play, multi-out, and third-out shapes through the ordinary offensive validator. The persisted batter identity and order size cannot change, and the batter plus every occupied event-time runner must appear exactly once with a legal destination. Every non-third-out home touch counts. On the third out, force/batter-runner classification counts no runs; timing classification requires the exact event-time home-touch sources that legally scored. RBI may not exceed counted runs, and zero-RBI scoring remains distinct. Missing, duplicate, unexpected, backward, passing, colliding, excess-out, illegal-sacrifice, run-source, RBI, ambiguous or contradictory third-out classification, and result/destination mismatches are rejected with a specific reason. A valid replacement preserves the record envelope and full replay applies score before any half-inning transition, clears count/bases/outs, preserves the next tracked batter, revalidates downstream history, and rebuilds player-attributed R/RBI plus the remaining batting projection; rejected later plate appearances require explicit staged repair before the batch can save atomically.
- Editing an earlier tracked-team SB/CS selects only a runner who occupied an eligible base in the event-time state. SB must advance 1B→2B, 2B→3B, or 3B→Home; CS must end at Out. The attempt never advances the active batter or changes the count. A steal of home adds one team score, one runner run, and one SB with zero RBI. A third-out CS advances the half-inning while preserving the next tracked batter. Wrong runner, source, destination, half, stale timeline, or rejected downstream history disables Save; downstream SB/CS requires an explicit staged repair, and a clean batch replaces exact payloads atomically without changing record envelopes.
