...
- Editing an earlier completed tracked-team plate appearance is limited to plays that do not make the third out. Walk, HBP, strikeout, hit, home-run, error, fielder's-choice, ordinary-out, sacrifice, and double-play shapes reuse the ordinary offensive validator. The persisted batter identity and order size cannot change, and the batter plus every occupied event-time runner must appear exactly once with a legal destination. Every non-third-out home touch counts, counted sources identify the exact event-time players credited with runs, and RBI may not exceed counted runs; zero-RBI scoring remains distinct. Missing, duplicate, unexpected, backward, passing, colliding, excess-out, illegal-sacrifice, run-source, RBI, third-out, and result/destination mismatches are rejected with a specific reason. A valid replacement preserves the record envelope and full replay rebuilds score, count, bases, outs, next batter, downstream validity, and player-attributed R/RBI plus the remaining batting projection; rejected later plate appearances require explicit staged repair before the batch can save atomically.
