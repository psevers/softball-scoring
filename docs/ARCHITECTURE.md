...
- `GameEventCorrection` is the adjacent main-actor correction boundary. Its Undo operation uses an isolated fresh SwiftData context to fetch one game, captures an exact expected timeline, previews removal of the latest eligible defensive pitch, completed Ball In Play result, tracked-team count pitch, completed tracked-team plate appearance, or tracked-team SB/CS event through the same snapshot/replay/projection path, rejects stale confirmation, and atomically deletes one record with rollback on save failure. Ball four, strike three, and HBP remain single authoritative pitch records, so replay restores their count, batter, runner, score, out, half-inning, and pitcher-total consequences together. Removing a completed Ball In Play result preserves its preceding counted pitch and restores the pending-result state. Removing a tracked-team count pitch preserves its event-time batter context and leaves player-attributed batting projection unchanged. Removing a completed tracked-team plate appearance restores its pre-play state and reprojects batting attribution from the surviving history. Removing SB/CS resolves the runner's event-time identity from accepted history, restores the pre-attempt base/out/half-inning state, and reprojects only that runner's SB/CS and steal-of-home run attribution without changing the active plate appearance. Pending UI-context changes cannot become the candidate or join the correction save.
...
- Slice 6: read-only Play History and Undo latest eligible action, including terminal defensive ball four, strike three, HBP, completed Ball In Play results, tracked-team count pitches, completed tracked-team plate appearances, and tracked-team SB/CS events, are delivered; earlier-event edit/delete and pitch-total reconciliation remain later work on the correction boundary.
