...
- Earlier tracked-team pitch edits use that same correction session. The caller supplies only the replacement result; candidate encoding reuses the persisted event-time player identity, lineup slot, and batting-order size before full replay validates the historical count, later batter transitions, and batting projection.
