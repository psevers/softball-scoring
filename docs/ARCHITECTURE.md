...
- Eligible completed tracked-team plate appearances use the same correction session for scoring or non-scoring replacements that do not make the third out. The editor derives the batter and occupied-runner identities from the event-time replay entry, accepts a scorer-confirmed result, complete runner map, exact counted home-touch sources, and RBI through the ordinary offensive validator, and replaces only the encoded event body. Candidate replay reprojects score, PA/AB/R/H/2B/3B/HR/RBI/BB/HBP/SO/sacrifice attribution and next-batter/base/out state. A later rejected tracked-team plate appearance can join the session as a second explicit replacement; Save writes all replacements once or rolls the batch back.
