...
- Pitch-count reconciliation uses the same correction boundary to append a starting-pitcher-addressed event after exact-timeline validation. Replay applies its signed total, ball, and strike adjustments while preserving live count and every non-pitch value. The event is explicit in History and eligible for latest-action Undo; stale history or save failure persists nothing partially.
