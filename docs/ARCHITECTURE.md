...
GameEventBody.pitchCountReconciliation(PitchCountReconciliationEvent)
...
Unknown kinds, malformed payloads, invalid or duplicate sequence numbers, semantic rejections, and
event-addressable batting-projection failures retain their event-time replay position in the trace so
Play History can show explicit problem entries.
...
- pitch total/ball/strike/unclassified counts by pitcher
...
- `GameEventCorrection` is the adjacent main-actor correction boundary. It uses an isolated fresh SwiftData context to fetch one game, captures an exact expected timeline, and validates every candidate through the same snapshot/replay/projection path. Undo previews removal of the latest eligible defensive pitch, completed Ball In Play result, tracked-team count pitch, completed tracked-team plate appearance, or tracked-team SB/CS event. Earlier edits, deletions, logical-play repairs, and already-locked history all share one correction session: every staged change retains exact record identity, the complete candidate replays after each change, and the first remaining problem carries stable sequence and decoded context when available into the repair UI. Known decodable events use only their supported editor or deletion control; unknown kinds, malformed payloads, and invalid or duplicate sequences allow deletion of only the exact problem record. Batting projection failures that identify an event sequence become explicit repairable problem entries. Durable history remains untouched while staging, and no repair silently skips, rewrites, reorders, renumbers, detaches, or cascade-deletes a survivor. Ball In Play replacement preserves the counted pitch component and changes only the result payload after the scorer reconfirms every event-time runner destination, legally counted run, RBI, and any decisive third-out classification. Defensive logical-play deletion removes the paired In Play pitch and result records together. Tracked-team logical-play deletion removes all count-pitch components plus the terminal plate-appearance result while preserving unrelated base-running events. Both retain individual component actions and rebuild state and projections exclusively through candidate replay. One clean session atomically applies every staged encoded-payload update and exact deletion; stale confirmation, projection failure, or save failure rolls the entire batch back. Ball four, strike three, and HBP remain single authoritative pitch records, so Undo replay restores their count, batter, runner, score, out, half-inning, and pitcher-total consequences together. Removing a completed Ball In Play result preserves its preceding counted pitch and restores the pending-result state. Removing a tracked-team count pitch preserves its event-time batter context and leaves player-attributed batting projection unchanged. Removing a completed tracked-team plate appearance restores its pre-play state and reprojects batting attribution from the surviving history. Removing SB/CS resolves the runner's event-time identity from accepted history, restores the pre-attempt base/out/half-inning state, and reprojects only that runner's SB/CS and steal-of-home run attribution without changing the active plate appearance. Pending UI-context changes cannot become a correction candidate or join its save.
- Pitch-count reconciliation uses the same correction boundary to append a starting-pitcher-addressed event after exact-timeline validation. The editor may associate it with one earlier accepted completed defensive play from the same game. That link persists the target record envelope plus a canonical payload-revision digest, so replay can reject missing, cross-game, unreadable, non-defensive, edited, or deleted targets without guessing. History groups a valid associated reconciliation into the related logical play while retaining the reconciliation's own sequence component and signed total/ball/strike/unclassified explanation. Replay applies only the signed pitcher-total adjustment; live count and every non-pitch value stay unchanged. A broken link enters the ordinary correction session and requires explicit re-association or association removal before the batch can save atomically; when deleting its play also makes a negative adjustment invalid, the scorer can explicitly delete the affected reconciliation in that same batch. The reconciliation remains latest-action Undo eligible; stale history or save failure persists nothing partially.
...
- Eligible completed tracked-team plate appearances use the same correction session for scoring, non-scoring, multi-out, and third-out replacements. The editor derives the batter and occupied-runner identities from the event-time replay entry, accepts a scorer-confirmed result, complete runner map, exact counted home-touch sources, RBI, and any force/batter-runner or timing-play third-out classification through the ordinary offensive validator, and replaces only the encoded event body. Candidate replay scores legal timing-play runs before the half-inning transition, clears count/bases/outs when required, preserves the next tracked batter, and reprojects PA/AB/R/H/2B/3B/HR/RBI/BB/HBP/SO/sacrifice attribution. A later rejected tracked-team plate appearance can join the session as a second explicit replacement; Save writes all replacements once or rolls the batch back.
- Accepted tracked-team SB/CS events use the same correction session. The editor derives the event-time eligible runners and their source bases from replay, supports changing SB to CS or CS to SB and selecting a different eligible runner, and replaces only the encoded event body. The ordinary base-running validator rejects wrong runner, source, destination, half, or impossible attempt shapes. Candidate replay preserves the active batter and count, attributes steal-of-home score/run/SB with zero RBI, advances the half after a third-out CS while retaining the next tracked batter, and requires explicit repair of a rejected downstream SB/CS before one atomic save.
...
- Slice 6: Play History, Undo latest eligible action, staged earlier defensive and tracked-team pitch editing and deletion, tracked-team SB/CS correction, completed tracked-team plate-appearance correction including multi-out and classified third-out plays, completed defensive Ball In Play correction including multi-out and classified third-out plays, defensive and tracked-team logical-play deletion, atomic multi-change downstream repair, starting-pitcher total reconciliation with defensive-play association, unreadable recovery, and chained repair of already-rejected history are delivered.
- The Slice 6 recovery journey composes those shipped boundaries without a test-only recovery path: it records mixed history, performs Undo and several independent corrections, repairs more than one locked problem in one atomic save, resumes scoring, terminates the app, and verifies the same replayed game state, batting projection, pitcher totals, History, and unlocked scoring after relaunch.
