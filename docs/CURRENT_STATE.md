...
Window 1 continuation — **Slice 5.5 scorebook visual alignment remains complete and approved. Slice 6 Play History, latest eligible-action Undo, earlier defensive and tracked-team pitch correction and deletion, eligible completed tracked-team plate-appearance correction, completed Ball In Play correction through multi-out and classified third-out plays, logical-play deletion, and atomic multi-change downstream repair are implemented through ticket #22.**
...
Eligible non-third-out tracked-team plate appearances now expose Edit Play in History whether or not they scored. The editor names the event-time player and batting-order context, shows current and proposed score plus all affected run/RBI batting lines, and uses native result, runner-destination, and RBI controls at standard and Accessibility XL sizes. The scorer can replace walks, HBP, strikeouts, hits, home runs, errors, fielder's choices, ordinary outs, sacrifices, and double plays while confirming every event-time runner exactly once. Candidate replay preserves the record envelope; recalculates score, bases, outs, count, next batter, and PA/AB/R/H/2B/3B/HR/RBI/BB/HBP/SO/sacrifice attribution; and offers a later rejected plate appearance as a second staged repair. Movement, run-source, and RBI validation identify the exact problem while Save remains disabled. Stale, invalid, projection, and save failures are atomic; focused UI coverage removes a run from a scoring double, verifies both affected batting lines, and verifies relaunch persistence.
