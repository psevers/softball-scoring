...
Window 1 continuation — **Slice 5.5 scorebook visual alignment remains complete and approved. Slice 6 Play History, latest eligible-action Undo, earlier defensive pitch correction, completed single-out/non-third-out Ball In Play correction including runs and RBI, and atomic multi-change downstream repair are implemented through ticket #35.**
...
Completed defensive Ball In Play results now expose Edit Play beside their paired counted In Play pitch when the original play records at most one out and does not create the third out. The scorer chooses a corrected hit, home run, error, fielder's choice, ordinary out, sacrifice bunt, or sacrifice fly and reconfirms every event-time runner destination. For a non-third-out play, the scorer explicitly confirms that every apparent home touch counts and may assign RBI up to, but not necessarily equal to, the counted runs. Candidate replay recalculates score, bases, outs, opponent batter slot, pitcher totals, and every later event before Save. Any later eligible scoring or non-scoring play result can join the same staged correction when replay rejects it. Every replacement preserves its result record envelope and counted pitch; invalid run quantity, RBI, movement, downstream replay, stale history, or save failure leaves durable history unchanged. Multi-out plays and plays that create the third out remain successor work.
