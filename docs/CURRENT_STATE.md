...
Eligible no-run, non-third-out tracked-team plate appearances now expose Edit Play in History. The editor names the event-time player and batting-order context, shows current and proposed summaries, and uses native result and runner-destination controls at standard and Accessibility XL sizes. The scorer can replace walks, HBP, strikeouts, hits, errors, fielder's choices, ordinary outs, sacrifice bunts, and double plays while confirming every event-time runner exactly once. Candidate replay preserves the record envelope, recalculates bases/outs/count/next batter and PA/AB/H/2B/3B/BB/HBP/SO/sacrifice attribution, and offers a later rejected plate appearance as a second staged repair. Stale, invalid, projection, and save failures are atomic; focused UI coverage corrects hit to error to out and verifies both relaunches.
