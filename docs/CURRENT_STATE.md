...
Window 1 continuation — **Slice 5.5 scorebook visual alignment remains complete and approved. Slice 6 Play History, latest eligible-action Undo, earlier defensive pitch correction, completed non-scoring Ball In Play correction, and atomic multi-change downstream repair are implemented through ticket #21.**
...
Completed non-scoring defensive Ball In Play results now expose Edit Play beside their paired counted In Play pitch. The scorer chooses a corrected single, double, triple, error, fielder's choice, ordinary out, or sacrifice bunt and reconfirms every event-time runner destination through the existing movement sheet. Candidate replay recalculates bases, outs, opponent batter slot, pitcher totals, and all later events before Save. If replay rejects a later eligible non-scoring play result, that result can join the same staged correction before the batch saves atomically. The original event-time play must itself be eligible, so a scoring, multi-out, or third-out record cannot enter this editor through downstream repair. Every replacement preserves its result record envelope and counted pitch; invalid movement, downstream replay, stale history, or save failure leaves durable history unchanged. Home touches, RBI, multi-out plays, and third-out classification remain successor work.

Earlier defensive pitch edits, pitch deletions, and non-scoring play replacements now open one in-memory defensive-event correction session. If replay rejects a later record, the problem summary preserves its sequence and event context, explains the state replay reached, and navigates to the affected history entry. Supported downstream pitch changes or eligible play replacements join the same staged list by exact record identity; every addition reruns full replay and batting projection. Cancel discards the entire session. Save remains disabled until the candidate is clean, then one isolated SwiftData save applies the whole batch. Stale history, projection failure, and simulated save failure leave every original record intact. Fresh-context, cold-store, and Accessibility XXXL tests verify the repaired count, pitcher totals, batting projection, history, and return to live scoring.
