...
Play History now opens a native pitch-total reconciliation editor for the current starting pitcher. The scorer enters signed total, ball, and strike adjustments while the editor derives the unclassified remainder and previews resulting totals. Save is unavailable for negative or over-classified results. A valid correction appends one authoritative event at the next durable sequence, preserves the live count and every non-pitch state or projection value, and appears as an explicit signed History entry. The latest reconciliation is Undo-eligible. Exact-timeline validation, rollback, fresh-context replay, process relaunch, and focused positive, negative, classified, and unclassified UI coverage preserve the established correction guarantees.
