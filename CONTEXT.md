...
The latest persisted scoring action when it is an eligible defensive Ball, Called Strike, Swinging Strike, Foul, HBP, completed Ball In Play result, or non-terminal tracked-team count pitch. A terminal-pitch candidate records the plate appearance it completed; a Ball In Play result candidate identifies the preceding counted pitch that will remain; a tracked-team pitch candidate retains the event-time player identity and batting-order size. Every candidate retains the expected game timeline so a moved or stale action cannot be removed.
